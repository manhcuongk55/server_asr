...
NORM_URL = 'http://10.208.209.175:11992/norm'
...
def post_process_json(str):
    try:
        ...
        if "result" in event:
            ...
            response = requests.post(NORM_URL, json={'sample': event["result"]["hypotheses"][0]["transcript"]})
            if response.status_code == 200:
               result = json.loads(response.text)
               text_norm = result['result'] 
               event["result"]["hypotheses"][0]["transcript"] = text_norm
            else:
